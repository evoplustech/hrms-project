...
const AttendanceTimesView = ({data,employeedata}) => {
  ...
  const Heading = ['Date','Day',' Day Status','In Time','Out Time','Hrs Worked','Action'];
//   ,'Shift Name'
    ...
    <>
      <div className="relative overflow-x-auto shadow-md mt-6">
            ...
            <table className="w-full text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead className="text-gray-900 capitalize bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr className='bg-slate-300'>
                      {
                        Heading.map((value)=>{
                          return <th key={value} scope="col" className="px-6 py-3  text-base font-bold">
                              {value}
                          </th>
                        })
                      }
                  ...
                  data.length > 0 &&  data.map((value)=>{
                    ...
                    const dayName = format(new Date(value.date),'EEEE');
                     return  (<tr key={value.date} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600  text-xl font-semibold">
                        ...
                        <td className="px-6 text-sm font-normal text-gray-700 tort">
                            {value.checkInTime || "00:00:00"}{value.status.toLowerCase()==='late-in' && <img src='/tortis.png'/>}
                        </td>
                        <td className="px-6  text-sm font-normal text-gray-700">
