...
const PicklistData = ({selectedOption,value}) => {
  ...
  return (
    <>
      ...
      <li className="hover:bg-zinc-100 cursor-pointer flex justify-between border py-2 px-4"><span>{value.name || ''}</span><div className="flex justify-between space-x-6"><RiDeleteBin2Line className="text-rose-500 w-5 h-5" onClick =  {popupHandler}/><FaRegEdit className="w-5 h-5 text-teal-600" onClick ={()=>setPopup(!popup)}/></div></li>
    </>
  )
}
