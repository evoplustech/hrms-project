...
import { employeePersonalDetail } from '../../../slices/employeeSlice';
...
const PersonalDetailsForm = () => {
 ...
 const formSubmitHandler = async (data)=>{
        const {street,city,state,zipCode,country,name,relationship,emergency_phone,aadharCard,passportNumber,panCard,phone,email} = data;
        ...
        try{
            const response = await dispatch(employeePersonalDetail(formData));
            if(response.payload?.success){
              const {success,data:{_id}} = await response.payload;
              
              navigate(`/home/employee/createEmployee/${_id}`);
            }else{
      ...
      <form className="max-w-7xl" encType="multipart/form-data" onSubmit = {handleSubmit(formSubmitHandler)}>
        ...
        <div className="grid  md:grid-cols-1 mt-20 ">
            <div className="flex justify-center mt-20">
              <button className="w-40 text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2"> Save & Continue</button>
            </div>
        </div>   
    </form>
    </div>
