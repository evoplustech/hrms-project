...
// form submit handler
  ...
  const formSubmitHandler = async (data)=>{
    ...
    const [loaddept,deptList] = useGetData({path : '/api/configure/picklist/department/getAll',method:'get'});
    const [loadrole,rolesList] = useGetData({path : '/api/configure/picklist/role/getAll',method:'get'});
    const [loaddesig,desigList]= useGetData({path : '/api/configure/picklist/designation/selectDesig',method:'get',params:watch('department')});
    const [loadManagers,reportingList]= useGetData({path : '/api/employee/getreportingmanagers',method:'get'});
    const [loadShift,shiftList] = useGetData({path:'/api/shift/AllShifts',method:'get'});
    ...
    <>
      <form className="max-w-7xl space-y-16" encType="multipart/form-data" onSubmit = {handleSubmit(formSubmitHandler)}>
        ...
        <div className="grid  md:grid-cols-3">
          ...
          <div>
            <Controller control = {control}  name="confirmation" defaultValue={false} render={({field})=>{
              return <Checkbox label='Confirmation : '   {...field}/>
            }} >
            </Controller>
          </div> 
        </div>
