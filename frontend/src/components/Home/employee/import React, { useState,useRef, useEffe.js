...
const ProfessionalForm = ({params,path,method,button,className}) => {
  ...
  const desigList =  designationData['data'].filter((value)=> value.department === department._id);
  ...
  const schema = z.object({
    ...
    console.log('locationnnnnnnnn',locationPath);
  ...
  const {register,handleSubmit,formState : {errors},setValue,watch,control } = useForm({
    ...
    defaultValues :{
      email,employeeId,dateOfJoining,employmentType,conformation,city,office,managerId,basic,allowances,hra,total
    }
  })
  ...
  <>
    <form className="max-w-7xl space-y-16" encType="multipart/form-data" onSubmit = {handleSubmit(formSubmitHandler)}>
      ...
      <div className="grid  md:grid-cols-4">
        ...
        <div className="">
          <Input label='Password : '  type="hidden"  name="password" {...register('password')} />
          {errors?.password && <p className="text-red-600 w-5">{errors.password.message}</p>}
        </div>
        <div className="">
          <Input label='Confirm Password : '  type="hidden"  name="confirmPassword" {...register('confirmPassword')} />
