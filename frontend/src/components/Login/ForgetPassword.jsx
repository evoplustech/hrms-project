        ...
        <p className="mb-6 mt-0 pb-2 flex justify-center items-center text-gray-600 text-xl font-semibold">Request Password</p>
          <form className="space-y-4" onSubmit = {forgetformHandler}>
            <div className="mx-0">
              ...
              <input onChange={(e)=>(setUsername(e.target.value))} value={username}  type="text" placeholder=' abc@xyz.com' className="m-2 p-4 w-full h-10 rounded-md bg-yellow-100 login-field" />
            </div>
            <Link to="/" className="hover:text-blue-500 hover:cursor-pointer text-xs">Back To Login</Link>
            <button disabled={loading?true:false} className='login-btn w-full p-2 mt-4 rounded-md text-md'>   
                {
