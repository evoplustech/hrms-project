...
const LoginForm = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center login-form-wrapper">
      <div  className=' max-w-md w-full p-6 rounded-lg shadow-md bg-gray-100 bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-0'>
        ...
        <form className="space-y-4" onSubmit = {loginFormSubmit}>
          <div className="">
            ...
            <input onChange={(e)=>(setUsername(e.target.value))} value={username}  type="text" placeholder='Enter username' className="m-2 p-4 w-full h-10 rounded-md login-field" />
          </div>
          <div className="relative w-full">
            <label className="block text-base font-semibold text-gray-700 mb-1">
              Password
            </label>
            <input ref={passref} onChange={(e)=>(setPassword(e.target.value))} value={password} type="password" placeholder='Enter password' className="m-2 ps-2 pe-6 w-full h-10 rounded-md login-field" />
              <span className="absolute top-1/2 -right-1 transform mt-4  -translate-y-1/2">{
                togglehide?<RiEyeLine className='eyecon hover:cursor-pointer w-5 h-5 z-50' onClick = {()=>(toggleHandler(!togglehide))} />:<RiEyeOffLine className='eyecon hover:cursor-pointer w-5 h-5 z-50' onClick = {()=>(toggleHandler(!togglehide))} />
              }</span>
          </div>
          <div>
            <div>
